...
from anime.services.file_utils import FileSearcher
...
class AnimeValidateSerializer(AuthorValidateSerializer):
    ...
    def validate_producer(producer):
        filtered_producers = Producer.objects.filter(id__in=producer)
        if len(filtered_producers) != len(producer):
            raise ValidationError('Producer not found!')
        return producer

    @staticmethod
    def validate_genre(genre):
        filtered_genres = Genre.objects.filter(id__in=genre)
        if len(filtered_genres) != len(genre):
            raise ValidationError('Genre not found!')
        return genre

    @staticmethod
    def validate_theme(theme):
        filtered_themes = Theme.objects.filter(id__in=theme)
        if len(filtered_themes) != len(theme):
            raise ValidationError('Producer not found!')
        return theme
...
class AnimeCreateSerializer(AnimeValidateSerializer):
    ...
    def validate_slug(slug):
        if Anime.objects.filter(slug=slug).count() > 0:
...
class SeasonValidateSerializer(AuthorValidateSerializer):
    ...
    voice_acting = serializers.ListField(
        child=serializers.IntegerField(min_value=1)
    )
    producer = serializers.ListField(
        child=serializers.IntegerField(min_value=1))
    ...
    def validate_voice_acting(voice_acting):
        filtered_voice_actings = VoiceActing.objects.filter(
            id__in=voice_acting
        )
...
class SeasonCreateSerializer(SeasonValidateSerializer):
    @staticmethod
    def validate_title(title):
        if AnimeSeason.objects.filter(title=title).count() > 0:
...
class EpisodeCreateSerializer(EpisodeValidateSerializer):
    @staticmethod
    def validate_title(title):
        if AnimeEpisode.objects.filter(title=title).count() > 0:
...
class ReviewValidateSerializer(AuthorValidateSerializer):
    ...
    rating = serializers.ChoiceField(
        choices=ANIME_RATING, allow_blank=False
    )
    anime = serializers.IntegerField(min_value=1)
